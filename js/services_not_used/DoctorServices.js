...
angular.module('DoctorServices', [])
  ...
  .factory('Doctor', ['$resource', function ($resource) {
    return $resource('data/doctor.json', { id: '@_id'}, {
        save: { 
          method: 'POST', 
  ...
  .factory('doctorFormStepsFactory', function(){
    ...
    factory.getSteps = function(){ 
      return [
        {
            templateUrl: './partials/doctors/form/step1.html',
            title: 'Basic info'
        },
        {
            templateUrl: './partials/doctors/form/step2.html',
            title: 'Background'
        },
        {
            templateUrl: './partials/doctors/form/step3.html',
            title: 'Commitment'
        },
        {
            templateUrl: './partials/doctors/form/submit.html',
            title: 'Submit'
        }
      ]
    };
  ...
  .factory('doctorMetadataFactory', function(doctorRemoteDataRequestFactory){
    ...
    factory.getData = function(){    
      ...
      doctorRemoteDataRequestFactory.getMetadata().success(function(response){
        ...
        metadata.all_countries = response.countries;
    ...
    factory.mapNames = function(response, doctorData, $scope){
      ...
      for(var i in response.all_countries)
      {
        if(response.all_countries[i].code === doctorData.country){
          $scope.doctor.country_name = response.all_countries[i].name;
          break;
        }     
      }
      ...
      for(var i in response.all_countries)
      {
        if(response.all_countries[i].code === doctorData.background.place_of_medical_degree.country){
          $scope.doctor.background.place_of_medical_degree.country_name = response.all_countries[i].name;
          break;
        }     
      }
      for(var i in response.all_countries)
      {
        if(response.all_countries[i].code === doctorData.background.place_of_residency.country){
          $scope.doctor.background.place_of_residency.country_name = response.all_countries[i].name;
          break;
        }     
      }
            for(var i in response.all_countries)
      {
        if(response.all_countries[i].code === doctorData.background.place_of_fellowship.country){
          $scope.doctor.background.place_of_fellowship.country_name = response.all_countries[i].name;
          break;
        }     
      }
